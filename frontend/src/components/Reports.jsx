...
import { 
    ...
    CalendarMonth, Person, AttachMoney 
} from '@mui/icons-material';
import API from '../api/axiosInstance'; 
import QRCode from 'react-qr-code';
import { createRoot } from 'react-dom/client';

// --- CONFIGURACIÓN DE ICONOS PARA EL RECIBO ---
        ...
        <>
            ...
            <TableRow sx={{ '& > *': { borderBottom: 'unset' }, backgroundColor: open ? '#f5f5f5' : 'inherit' }}>
                ...
                <TableCell align="right">
                    <Chip label={`${row.items.length} ítems`} size="small" />
                </TableCell>
                <TableCell align="right">
            ...
            {/* Fila Desplegable (Los Productos) */}
            <TableRow>
...
const SalesHistory = () => {
    ...
    useEffect(() => {
        const fetchSales = async () => {
            ...
            try {
                ...
                // Asumiendo que esta ruta devuelve la lista plana de items como en tu foto
                const response = await API.get('/inventory/sales-history', {
                    headers: { Authorization: `Bearer ${token}` }
                });
                
                // 2. AGRUPAR DATOS (Magia Frontend)
                // Si el backend manda items sueltos, los agrupamos por fecha/ID aquí.
    ...
    // Función para agrupar la lista plana de items en ventas únicas
    const groupSalesData = (flatItems) => {
        const groups = {};
        
        flatItems.forEach(item => {
            // Usamos ticket_id si existe, o creamos una clave única con la fecha exacta + vendedor
            // (Asegúrate de que tu backend envíe un ID único por venta si es posible)
            const key = item.ticket_id || `${item.fecha_hora}_${item.vendedor}`;
            
            if (!groups[key]) {
                groups[key] = {
                    id: key,
                    fecha: item.fecha_hora || item.created_at, // Ajusta según tu DB
            ...
            groups[key].items.push({
                producto: item.producto, // Ajusta nombres según tu DB
                codigo: item.codigo,
            ...
            // Sumamos al total de esa venta
            groups[key].totalVenta += (item.precio_unitario * item.cantidad); // O usa item.total_venta si ya viene calculado
        });
    ...
    // --- FUNCIÓN DE IMPRESIÓN (Idéntica al POS pero adaptada) ---
    const handleReprint = async (saleRow) => {
        try {
            ...
            // Mapeamos los datos del historial al formato que espera el ticket
            ...
            const ticketId = saleRow.id.toString().substring(0, 15); // Un ID corto
